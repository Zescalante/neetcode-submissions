...
class Solution:
    def solve(self, board: List[List[str]]) -> None:
        ...
        directions = [[1,0],[0,1],[-1,0],[0,-1]]    #move in 4 directions

        def dfs(i, j):
            # if out of range, or cell isn't O, then return
            if min(i, j) < 0 or (i > rows - 1) or (j > cols - 1) or \
            board[i][j] == 'X' or board[i][j] == '*':
                return 
            
            board[i][j] = '*'   #temp mark the 'O' as * to change later

            for d in directions:    #recurse in each direction
                dfs(i + d[0], j + d[1])
        ...
        for r in range(rows):   #change the temp * back to O, and change the remaining O to X 
            for c in range(cols):
                if board[r][c] == 'O':
                    board[r][c] = 'X'
                elif board[r][c] == '*':
